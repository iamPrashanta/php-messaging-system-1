...
$msg_from_list = mysqli_query($msgconn, "SELECT DISTINCT `msg_from`, `msg_to` FROM `messages` WHERE `msg_to` = $Login_ID OR msg_from = $Login_ID;");
...
if (mysqli_num_rows($msg_from_list) > 0) {
    while ($rows = mysqli_fetch_assoc($msg_from_list)) {
        ...
        if ($Login_ID == $msg_from) {
            ...
            WHERE `msg_from`= '$msg_from' AND `msg_to`='$Login_ID' AND `msg_read`='0';"));
